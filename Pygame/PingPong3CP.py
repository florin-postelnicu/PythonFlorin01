...
pygame.mixer.music.load("//ahs-wl.puhsd.loc/staff/postelnicu/Desktop/ResourcesPy/fears.mp3")
...
while not done:
    ...
    # Computer Player= Blue Player
    if (math.fabs(ball_x - plat2.x) < 100):
        plat2_y = (ball_y - 50)*math.cos(math.pi/2*((math.fabs(ball_x - plat2.x)/100)))

    #
    # if (math.fabs(ball_x - plat1.x) < 100):
    #     plat1_y = (ball_y - 50) * math.cos(math.pi /2 * ((math.fabs(ball_x - plat1.x) / 100)))
